...
function switchEducationTab(tabName) {
    ...
    document.querySelectorAll('.edu-content').forEach(content => content.classList.remove('active'));
    
    // Add active class to selected tab and content
    const selectedTab = document.querySelector(`.edu-tab[data-tab="${tabName}"]`);
    const selectedContent = document.getElementById(`edu-${tabName}`);
 ...
 */
function toggleEducationCollapse(header) {
...
/**
 * Convert depth between meters and feet
 */
function convertDepth(from) {
    const metersInput = document.getElementById('depthMeters');
    const feetInput = document.getElementById('depthFeet');
    ...
    if (from === 'm') {
        ...
        const meters = (feet / 3.28084).toFixed(1);
...
function convertPressure(from) {
    const barInput = document.getElementById('pressureBar');
    const psiInput = document.getElementById('pressurePSI');
    
    if (from === 'bar') {
        const bar = parseFloat(barInput.value) || 0;
        const psi = (bar * 14.5038).toFixed(0);
        psiInput.value = psi;
    } else {
        const psi = parseFloat(psiInput.value) || 0;
        const bar = (psi / 14.5038).toFixed(0);
        barInput.value = bar;
    }
}
